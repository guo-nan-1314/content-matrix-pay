 ...
 *
 * 用法：
 *   node scripts/content-matrix-paid.mjs probe --action matrix-plan --industry 美妆 --accounts 3 --platforms xhs,douyin --goal brand_exposure --state-dir .state/session1
 *   node scripts/content-matrix-paid.mjs pay --state-dir .state/session1 --session-id session1
 *   node scripts/content-matrix-paid.mjs complete --state-dir .state/session1 --out-shake-no PAY_xxx
 */
...
const RESOURCE_URL = 'https://am-server-content-matrix-ofbzuzmvbr.cn-hangzhou.fcapp.run/api/content-matrix';
const SERVICE_PRICE = '0.50';
...
async function probe(args) {
  ...
  const action = args.action || 'matrix-plan';
  const body = { action };

  // 根据 action 收集参数
  if (args.industry) body.industry = args.industry;
  if (args.accounts) body.accounts = parseInt(args.accounts, 10);
  if (args.platforms) body.platforms = args.platforms;
  if (args.goal) body.goal = args.goal;
  if (args.team_size) body.team_size = parseInt(args.team_size, 10);
  ...
  if (args.weekly_capacity) body.weekly_capacity = parseInt(args.weekly_capacity, 10);
  if (args.impressions) body.impressions = parseInt(args.impressions, 10);
  if (args.clicks) body.clicks = parseInt(args.clicks, 10);
  if (args.likes) body.likes = parseInt(args.likes, 10);
  if (args.saves) body.saves = parseInt(args.saves, 10);
  if (args.comments) body.comments = parseInt(args.comments, 10);
  if (args.followers_gained) body.followers_gained = parseInt(args.followers_gained, 10);
  ...
  console.log(`   服务: 内容矩阵运营 Pro`);
  console.log('');
  console.log('下一步：运行 pay 命令完成支付');
  console.log(`   node scripts/content-matrix-paid.mjs pay --state-dir "${stateDir}" --session-id <SESSION_ID>`);
...
async function pay(args) {
  ...
  const botArgs = [
    ...
    '--intent-summary', `内容矩阵运营服务 - ${requestMeta.body.action || '未知操作'}`,
  ];
...
try {
  switch (command) {
    ...
    case 'complete':
      ...
      break;
    default:
      console.log('内容矩阵运营 Pro - 按量付费客户端');
      console.log('');
      console.log('用法:');
      console.log('  node scripts/content-matrix-paid.mjs probe --action <action> [options] --state-dir <dir>');
      console.log('  node scripts/content-matrix-paid.mjs pay --state-dir <dir> --session-id <id>');
      console.log('  node scripts/content-matrix-paid.mjs complete --state-dir <dir> --out-shake-no <order>');
      console.log('');
      console.log('操作类型 (action):');
      console.log('  matrix-plan        账号矩阵规划');
      console.log('  content-generate   爆款内容生成');
      console.log('  schedule-optimize  发布排期优化');
      console.log('  data-analyze       数据复盘分析');
      console.log('');
      console.log('示例:');
      console.log('  node scripts/content-matrix-paid.mjs probe --action matrix-plan --industry 美妆 --accounts 3 --platforms xhs,douyin --goal brand_exposure --state-dir .state/s1');
      console.log('  node scripts/content-matrix-paid.mjs pay --state-dir .state/s1 --session-id s1');
      console.log('  node scripts/content-matrix-paid.mjs complete --state-dir .state/s1 --out-shake-no PAY_xxx');
      process.exit(1);
  }
} catch (err) {
